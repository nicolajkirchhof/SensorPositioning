...
#ifndef BOOST_GEOMETRY_ALGORITHMS_DETAIL_HAS_SELF_INTERSECTIONS_NICO_HPP
#define BOOST_GEOMETRY_ALGORITHMS_DETAIL_HAS_SELF_INTERSECTIONS_NICO_HPP
...
#ifdef BOOST_GEOMETRY_DEBUG_HAS_SELF_INTERSECTIONS_NICO
...
inline bool has_self_intersections_nico(Geometry const &geometry, std::vector<boost::geometry::segment_identifier> *merge = NULL)
{
    ...
    for(typename std::deque<turn_info>::const_iterator it = boost::begin(turns);
        ...
        if (! valid) {
            ...
            if (merge != NULL) {
                if (info.operations[0].operation == detail::overlay::operation_blocked &&
                        info.operations[1].operation == detail::overlay::operation_union) {
                    merge->push_back(info.operations[0].seg_id);
                    merge->push_back(info.operations[0].other_id);
                } else if (info.operations[0].operation == detail::overlay::operation_intersection) {
                    merge->push_back(info.operations[0].seg_id);
                    merge->push_back(info.operations[0].other_id);
                } else if(
                    info.operations[1].operation == detail::overlay::operation_intersection) {
                    merge->push_back(info.operations[1].seg_id);
                    merge->push_back(info.operations[1].other_id);
                }
            }
...
#endif // BOOST_GEOMETRY_ALGORITHMS_DETAIL_HAS_SELF_INTERSECTIONS_NICO_HPP

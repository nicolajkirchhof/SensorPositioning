function [ y, J ] = aoaCos( state, opt_config )
...
for idx_sensor = 1:size(aoas, 1)
    for idx_measure = 1:size(aoas, 2)
        ...
        for idx_aoa = 1:2:numel(aoas{idx_sensor,idx_measure})
            ...
            y(cnt) = sin(phi+aoa)-cos(phi+aoa)*(py-sy)/(px-sx);
            if nargout > 1
                %for i = 1:scal.numEquations
                J(cnt, xOffset+idxs) = -(cos(aoa + phi)*(py - sy))/(px - sx)^2;
                J(cnt, yOffset+idxs) = cos(aoa + phi)/(px - sx);
                J(cnt, phiOffset+idxs) = cos(aoa + phi) + (sin(aoa + phi)*(py - sy))/(px - sx);
                J(cnt, xOffset+idxm)= (cos(aoa + phi)*(py - sy))/(px - sx)^2;
                J(cnt, yOffset+idxm) = -cos(aoa + phi)/(px - sx);
                %end
            end
            %             end
            cnt = cnt + 1;

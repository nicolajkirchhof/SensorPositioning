namespace GpcWrapper
{
	...
	public class VertexList
	{
        ...
        public VertexList(double[] x, double[] y)
        {
            NofVertices = x.Length;
            Vertex = new Vertex[NofVertices];
            for (int i = 0; i < x.Length; i++)
                Vertex[i] = new Vertex((double)x[i], (double)y[i]);
        }

		public VertexList( Vertex[] p )
		{
	...
	public class Polygon
	{
        ...
        public Polygon(VertexList contour, bool contourIsHole)
        {
            this.AddContour(contour, contourIsHole);
        }

        public Polygon(VertexList[] contours, bool[] contourIsHole)
        {
            for (int i = 0; i<contours.Length; i++)
            {
                this.AddContour(contours[i], contourIsHole[i]);
            }
        }
		...
		private static gpc_polygon PolygonTo_gpc_polygon( Polygon polygon )
		{
			...
			for ( int i=0 ; i<polygon.NofContours ; i++ ) {
				...
				for ( int j=0 ; j<polygon.Contour[i].NofVertices ; j++ ) {
					...
					ptr2 = ptr2 + Marshal.SizeOf(gpc_vtx) ;
				}
				Marshal.StructureToPtr( gpc_vtx_list, ptr, false );
				ptr = ptr + Marshal.SizeOf(gpc_vtx_list);
			}

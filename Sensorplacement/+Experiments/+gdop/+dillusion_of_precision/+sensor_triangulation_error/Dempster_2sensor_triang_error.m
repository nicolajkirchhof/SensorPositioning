...
figure; 
sz = {num_distances, 5};
all_axes = nan(1, num_distances*5);
cnt = 1;
for ids = 1:num_distances
...
all_axes(cnt) = subplot(sz{:},cnt); plot(sol.s1_s2_quality_dd_dop{ids}); title('dd_dop_quality')
cnt = cnt +1;
sol.s1_s2_quality_kelly{ids}(sol.s1_s2_quality_kelly{ids}>100) = 100;
all_axes(cnt) = subplot(sz{:},cnt); plot(sol.s1_s2_quality_kelly{ids}); title('kelly uncertainty')
xlim([0 180]);
cnt = cnt +1;
% kelly = 1./sin(deg2rad(phi));
% kelly(kelly>1e2) = 1e2;
% subplot(3,2,3); plot(kelly); title('1/sin(phi)');
all_axes(cnt) = subplot(sz{:},cnt); plot(sol.s1_s2_and_area{ids}); title('area');
xlim([0 180]);
cnt = cnt +1;
all_axes(cnt) = subplot(sz{:},cnt); plot(sol.s1_s2_and_length{ids}); title('length');
xlim([0 180]);
cnt = cnt +1;
all_axes(cnt) = subplot(sz{:},cnt); plot(sol.s1_s2_and_dmax{ids}); title('max point dist');
xlim([0 180]);
cnt = cnt +1;
end
%
...
arrayfun(@(h) ylim(h, [0 1]), all_axes(1:5:end));

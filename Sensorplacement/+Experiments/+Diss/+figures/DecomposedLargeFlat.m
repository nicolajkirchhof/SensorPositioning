...
%%%
cmcqm_sol = all_eval.large_flat.cmcqm_cmaes_it{306};
cmcqm_wpn = cmcqm_sol.solutions{end}.sp;
cmcqm_wpn(3, :) = cmcqm_wpn(3, :) + deg2rad(input.config.discretization.sensor.fov/2);
mb.drawPose(cmcqm_wpn, 900, 'color', 0.75*ones(1,3), 'linewidth', 1, 'markersize', sm/2, 'marker', 'd', 'markerfacecolor', 0.5*ones(1,3));

cmqm_sol = all_eval.large_flat.cmqm_cmaes_it{561};
cmqm_sp = cmqm_sol.solutions{end-1}.sp;
cmqm_sp(3, :) = cmqm_sp(3, :) + deg2rad(input.config.discretization.sensor.fov/2);
mb.drawPose(cmqm_sp, 750, 'color', 0.5*ones(1,3), 'linewidth', 1, 'markersize', sm/2, 'marker', 'd', 'markerfacecolor', 0.5*ones(1,3));

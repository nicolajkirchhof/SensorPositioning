...
% for ideval = 1:numel(eval_names)
    ...
    figure;
    h0 = barglyph(xlist, ylist, height, width, all_mean_wpn_qualities_rounded(ind, 3:6), xrange, yrange);
    ...
    h = legend({'GCO', 'GCSS', 'GSSS', 'STCM'},'Location', 'SouthOutside', 'Orientation','horizontal' );
    pos = get(h, 'position');
    set(h, 'position', [0.25 0 0.5 0.05])
    
    outfile = sprintf('%s%s_greedy_comparison.png', outdir, opts.eval_name);
    saveas(gcf, outfile);

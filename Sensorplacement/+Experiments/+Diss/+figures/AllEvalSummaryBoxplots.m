...
opt_names_print = {'{\scriptsize A}', '{\scriptsize B}', '{\scriptsize C}', '{\scriptsize D}', '{\scriptsize E}', '{\scriptsize J}', '{\scriptsize F}', '{\scriptsize G}', '{\scriptsize H}', '{\scriptsize I}'};%, '{\scriptsize J}'};
%%
...
boxplot(opts.all_mean_wpn_qualities(:, xrange)*100, 'labels', opt_names_print(xrange), 'colors', [0.4 0.4 0.4], 'symbol', 'k+');
...
% set x tick labels :
for i=1:size(opt_names_print(xrange),2)                 % number of box plot bars in figure
    text('Interpreter','none',...
        'String',opt_names_print{xrange(i)},...
        'Units','normalized',...
        'VerticalAlignment','top',...
        'HorizontalAlignment','center',...
        'Position',[1/(size(opt_names_print(xrange),2)*2)*(1+2*(i-1)),-0.01],...x/y position
        'EdgeColor','none')
end
...
figure, boxplot(opts.all_num_sp_selected(:, xrange), 'labels', opt_names_print(xrange), 'colors', [0.4 0.4 0.4], 'symbol', 'k+');
h = findobj(gcf,'Tag','Outliers');
% make median lines black and big
set(findobj(gcf,'Tag','Median'),'Color',[0 0 0],'LineWidth',2);
...
figure, boxplot(opts.all_area_covered_pct(:, xrange), 'labels', opt_names_print(xrange), 'colors', [0.4 0.4 0.4], 'symbol', 'k+');
h = findobj(gcf,'Tag','Outliers');
% make median lines black and big
set(findobj(gcf,'Tag','Median'),'Color',[0 0 0],'LineWidth',2);

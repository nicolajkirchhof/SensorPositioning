...
for id_n = 1:numel(names)
    ...
    for id_wpn = 1:numel(num_wpns)
        for id_sp = 1:numel(num_sps)
            ...
            gen.workdir = sprintf('tmp/bspqm');
            bspqm_config = Configurations.Optimization.Discrete.bspqm(gen);
            bspqm_config.name = sprintf('%d_%s', cnt, name);
            filename = Optimization.Discrete.Models.bspqm(input.discretization, input.quality, bspqm_config);

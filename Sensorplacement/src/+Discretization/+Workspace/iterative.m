...
%%
placeable_ring = mb.expandPolygon(environment.combined, -options.wall_distance);
...
%%
inplaceable = binpolygon(initial_positions, placeable_ring{1}, 1);
notinplaceable = cellfun(@(x) binpolygon(initial_positions, x, 1),  placeable_ring(2:end), 'uniformoutput', false);
flt_notinplaceable = cellfun(@(x) ~isempty(x), notinplaceable);
notinplaceables = cell2mat(notinplaceable(flt_notinplaceable)');


if ~isempty(notinplaceable)
    inplaceable = inplaceable&~(any(notinplaceables, 1));
end
%%
...
while options.positions.additional >= size([fullgrid_positions, refined_grid_in], 2)
    ...
    inplaceable = binpolygon(refined_grid, placeable_ring{1}, 1);
    notinplaceable = cellfun(@(x) binpolygon(refined_grid, x, 1),  placeable_ring(2:end), 'uniformoutput', false);
    flt_notinplaceable = cellfun(@(x) ~isempty(x), notinplaceable);
    notinplaceables = cell2mat(notinplaceable(flt_notinplaceable)');
...
inplaceable = binpolygon(sorted_positions, placeable_ring{1}, 1);
notinplaceable = cellfun(@(x) binpolygon(sorted_positions, x, 1),  placeable_ring(2:end), 'uniformoutput', false);
flt_notinplaceable = cellfun(@(x) ~isempty(x), notinplaceable);
notinplaceables = cell2mat(notinplaceable(flt_notinplaceable)');
